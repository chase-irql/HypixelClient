package com.teeko.enemyboxes.client.integration;
...
public final class BotEventClient {
    ...
    private static final ThreadPoolExecutor EVENT_EXECUTOR = new ThreadPoolExecutor(
            ...
            task -> {
                Thread thread = new Thread(task, "EnemyBoxes-BotEvents");
    ...
    public static boolean sendBeachballForcedStopEvent(
        ...
        AlertSettings settings = loadAlertSettings(() -> EnemyBoxesState.beachballForcedStopAlertsEnabled);
    ...
    public static boolean sendChatMentionEvent(
        ...
        AlertSettings settings = loadAlertSettings(() -> EnemyBoxesState.chatNameMentionAlertsEnabled);
        if (!settings.enabled || settings.eventEndpointUrl.isEmpty()) {
            return false;
        }
    ...
    public static boolean sendServerShutdownEvent(String playerName, String triggerMessage) {
        AlertSettings settings = loadAlertSettings(() -> EnemyBoxesState.serverShutdownAlertsEnabled);
        if (!settings.enabled || settings.eventEndpointUrl.isEmpty()) {
            return false;
        }
    ...
    private static void sendEvent(
        ...
        try {
            ...
            System.err.println("[EnemyBoxes] Bot event #" + eventId + " request error: " + error.getMessage());
        }
    }
    ...
    private static AlertSettings loadAlertSettings(BooleanSupplier enabledSupplier) {
        ...
        String serverUrl = sanitize(EnemyBoxesState.alertServerUrl);
        String sharedSecret = sanitize(EnemyBoxesState.alertServerSecret);

        if (!enabled || serverUrl.isEmpty()) {
            EnemyBoxesConfig.reloadAlertSettings();
            enabled = enabledSupplier.getAsBoolean();
            serverUrl = sanitize(EnemyBoxesState.alertServerUrl);
            sharedSecret = sanitize(EnemyBoxesState.alertServerSecret);
        }
    ...
    private static synchronized String getCachedAuthToken(PlayerIdentity playerIdentity) {
        String cachedToken = sanitize(EnemyBoxesState.alertAuthToken);
        String cachedMinecraftUuid = sanitize(EnemyBoxesState.alertAuthMinecraftUuid);
        ...
        try {
            ...
            for (int attempt = 1; attempt <= AUTH_REQUEST_RETRY_ATTEMPTS; attempt++) {
                ...
                try {
                    ...
                    System.err.println("[EnemyBoxes] Bot event #" + eventId + " auth retry interrupted.");
                    return "";
                }
            }
            ...
            System.err.println("[EnemyBoxes] Bot event #" + eventId + " auth request error: " + error.getMessage());
            return "";
        }
    }
    ...
    private static synchronized void cacheAuthToken(String minecraftUuid, String token) {
        ...
        if (cleanToken.equals(EnemyBoxesState.alertAuthToken)
                && cleanMinecraftUuid.equalsIgnoreCase(EnemyBoxesState.alertAuthMinecraftUuid)) {
            return;
        }

        EnemyBoxesState.alertAuthToken = cleanToken;
        EnemyBoxesState.alertAuthMinecraftUuid = cleanMinecraftUuid;
        EnemyBoxesConfig.save();
    }

    private static synchronized void clearCachedAuthToken() {
        if (EnemyBoxesState.alertAuthToken.isEmpty()
                && EnemyBoxesState.alertAuthMinecraftUuid.isEmpty()) {
            return;
        }

        EnemyBoxesState.alertAuthToken = "";
        EnemyBoxesState.alertAuthMinecraftUuid = "";
        EnemyBoxesConfig.save();
    }
