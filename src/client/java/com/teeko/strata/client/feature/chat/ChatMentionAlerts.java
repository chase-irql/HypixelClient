...
public final class ChatMentionAlerts {
    ...
    private static void handleDetectedPlayerMessage(
        ...
        if (containsMacroKeyword(cleanChatBody) && StrataState.beachballMacroRunning) {
            handleMacroCallout(client, cleanSenderName);
        }

        if (!StrataState.chatNameMentionAlertsEnabled) {
    ...
    private static void handleMacroCallout(Minecraft client, String senderName) {
        ...
        if (client.player != null) {
            client.player.displayClientMessage(
                    Component.literal(
                            "[Strata] Macro callout detected from "
    ...
    private static void debugLog(String message) {
        System.out.println("[Strata] Mention debug: " + message);
    }
