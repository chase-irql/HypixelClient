...
public final class StrataAim {
    ...
    private static void applyAim(Minecraft client, Vec3 aimPos) {
        ...
        Vec3 target = aimPos.add(
                driftX + (random.nextFloat() - 0.5f) * StrataState.jitterStrength,
                driftY + (random.nextFloat() - 0.5f) * StrataState.jitterStrength,
                0
        );
    ...
    private static float nextTriangularSmoothing() {
        float lo   = StrataState.aimSmoothingMin;
        float hi   = StrataState.aimSmoothingMax;
        float mode = StrataState.aimSmoothingMode;
    ...
    private static void updateDrift() {
        ...
        if (now >= nextDriftRetargetMs) {
            driftTargetX = (random.nextFloat() - 0.5f) * 2f * StrataState.driftStrength;
            driftTargetY = (random.nextFloat() - 0.5f) * 2f * StrataState.driftStrength;
