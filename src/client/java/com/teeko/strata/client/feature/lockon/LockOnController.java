package com.teeko.strata.client.feature.lockon;
...
public final class LockOnController {
    ...
    public static void updateLockOn(Minecraft client) {
        ...
        if (!StrataState.enabled || !StrataState.hasTarget()) return;

        if (StrataState.lockedTarget != null) {
            Entity lockedEntity = null;
            for (Entity entity : client.level.entitiesForRendering()) {
                if (entity.getUUID().equals(StrataState.lockedTarget)) {
                    lockedEntity = entity;
                    break;
                }
            }
        ...
        for (Entity entity : client.level.entitiesForRendering()) {
            ...
            double fovR = StrataState.lockFov;
        ...
        if (!candidates.isEmpty()) {
            ...
            float blend = StrataState.aimPriorityBlend;
        ...
        StrataState.lockedTarget = bestTarget;
    }
    ...
    public static void tickAutoSwing(Minecraft client) {
        ...
        if (isNewTarget && StrataState.randomizeReactionDelay) {
    ...
    public static void tickAutoHuntUse(Minecraft client) {
        ...
        UUID huntTarget = StrataState.autoHuntEnabled ? StrataState.huntLockedBullet : null;
        if (huntTarget == null) {
            resetHuntUseState();
            return;
        }
    ...
    private static UUID getAutoSwingTarget() {
        ...
        if (FishingCombat.isKilling() && StrataState.lockedTarget != null) {
            return StrataState.lockedTarget;
        }
        if (StrataState.autoSwingEnabled && StrataState.lockedTarget != null) {
            return StrataState.lockedTarget;
        }
        return null;
    }
