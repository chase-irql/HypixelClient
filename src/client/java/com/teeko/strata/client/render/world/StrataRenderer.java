package com.teeko.strata.client.render.world;
...
public final class StrataRenderer {

    private static final RenderPipeline BOX_PIPELINE = RenderPipelines.register(
            RenderPipeline.builder(RenderPipelines.LINES_SNIPPET)
                    .withLocation(Identifier.fromNamespaceAndPath(StrataClient.MOD_ID, "pipeline/enemy_box_through_walls"))
    ...
    public static void render(Camera camera, Matrix4f viewMatrix, float tickDelta) {
        ...
        // ── Regular ESP boxes ────────────────────────────────────────────────
        if (StrataState.enabled && StrataState.hasTarget()) {
            for (Entity entity : client.level.entitiesForRendering()) {
                if (entity == client.player) continue;
                if (!(entity instanceof LivingEntity living) || !living.isAlive()) continue;
                if (!StrataState.matches(living)) continue;
                ...
                boolean isLocked = StrataState.lockedTarget != null &&
                        StrataState.lockedTarget.equals(entity.getUUID());
        ...
        if (StrataState.autoHuntEnabled) {
            for (Entity entity : client.level.entitiesForRendering()) {
                ...
                boolean isBullet  = entity instanceof ShulkerBullet
                        ...
                        && entity.getUUID().equals(StrataState.huntTrackedBullet);

                if (!isShulker && !isBullet) continue;

                // Skip shulkers that ESP is already drawing to avoid double-draw
                if (isShulker && StrataState.enabled
                        && StrataState.matches((LivingEntity) entity)) continue;
                ...
                if (isShulker) {
                    boolean isActive = StrataState.huntLockedShulker != null
                            && StrataState.huntLockedShulker.equals(entity.getUUID());
                    ...
                    boolean isAimLocked = StrataState.huntLockedBullet != null
                            && StrataState.huntLockedBullet.equals(entity.getUUID());
        ...
        if (StrataState.beachballMacroRunning && beachballRenderId != null && !beachballRendered) {
            for (Entity entity : client.level.entitiesForRendering()) {
        ...
        if (StrataState.autoFisherEnabled && AutoFisher.originBlock != null) {
    ...
    private static void drawBoxes(Minecraft client) {
        ...
        if (vertexBuffer == null || vertexBuffer.size() < needed) {
            ...
            vertexBuffer = new MappableRingBuffer(
                    () -> StrataClient.MOD_ID + " enemy box vbo",
        ...
        try (RenderPass pass = RenderSystem.getDevice()
                ...
                .createRenderPass(
                        () -> StrataClient.MOD_ID + " enemy box pass",
