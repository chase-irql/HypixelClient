...
public final class AutoClicker {
    ...
    public static void tick(Minecraft client) {
        if (!StrataState.autoClickerEnabled
                || !StrataClient.isPlayerActive(client)
                || client.level == null
                || client.player == null
                || client.gameMode == null
                || !client.options.keyAttack.isDown()
                || (StrataState.autoSwingEnabled && StrataState.lockedTarget != null)
    ...
    public static boolean shouldFireAutoSwing() {
        long now = System.currentTimeMillis();
        float minCps = delayMsToCps(StrataState.swingDelayMax);
        float modeCps = delayMsToCps(StrataState.swingDelayMode);
        float maxCps = delayMsToCps(StrataState.swingDelayMin);
    ...
    public static void fireAttack(Minecraft client) {
        ...
        if (minecraft.strata$getMissTime() > 0) {
            minecraft.strata$setMissTime(0);
        }
        minecraft.strata$invokeStartAttack();
    }
