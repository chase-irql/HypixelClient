package com.teeko.strata.client.integration;
...
public final class BotEventClient {
    ...
    private static final ThreadPoolExecutor EVENT_EXECUTOR = new ThreadPoolExecutor(
            ...
            task -> {
                Thread thread = new Thread(task, "Strata-BotEvents");
    ...
    public static boolean sendBeachballForcedStopEvent(
        ...
        AlertSettings settings = loadAlertSettings(() -> StrataState.beachballForcedStopAlertsEnabled);
        ...
        if (playerIdentity == null) {
            System.err.println("[Strata] Bot event not sent: unable to read Minecraft session.");
            return false;
        }
    ...
    public static boolean sendAutoFishForcedStopEvent(
        ...
        AlertSettings settings = loadAlertSettings(() -> StrataState.autoFisherForcedStopAlertsEnabled);
        if (!settings.enabled || settings.eventEndpointUrl.isEmpty()) {
            return false;
        }
    ...
    public static boolean sendChatMentionEvent(
        ...
        AlertSettings settings = loadAlertSettings(() -> StrataState.chatNameMentionAlertsEnabled);
        if (!settings.enabled || settings.eventEndpointUrl.isEmpty()) {
            return false;
        }
    ...
    public static boolean sendServerShutdownEvent(String playerName, String triggerMessage) {
        AlertSettings settings = loadAlertSettings(() -> StrataState.serverShutdownAlertsEnabled);
        if (!settings.enabled || settings.eventEndpointUrl.isEmpty()) {
            return false;
        }
    ...
    private static void sendEvent(
        ...
        try {
            ...
            if (bearerToken.isEmpty() && settings.sharedSecret.isEmpty()) {
                System.err.println("[Strata] Bot event #" + eventId
            ...
            System.out.println("[Strata] Bot event #" + eventId
            ...
            System.err.println("[Strata] Bot event #" + eventId + " request error: " + error.getMessage());
        }
    }
    ...
    private static AlertSettings loadAlertSettings(BooleanSupplier enabledSupplier) {
        ...
        String serverUrl = sanitize(StrataState.alertServerUrl);
        String sharedSecret = sanitize(StrataState.alertServerSecret);

        if (!enabled || serverUrl.isEmpty()) {
            StrataConfig.reloadAlertSettings();
            enabled = enabledSupplier.getAsBoolean();
            serverUrl = sanitize(StrataState.alertServerUrl);
            sharedSecret = sanitize(StrataState.alertServerSecret);
        }
    ...
    private static synchronized String getCachedAuthToken(PlayerIdentity playerIdentity) {
        String cachedToken = sanitize(StrataState.alertAuthToken);
        String cachedMinecraftUuid = sanitize(StrataState.alertAuthMinecraftUuid);
        ...
        try {
            ...
            for (int attempt = 1; attempt <= AUTH_REQUEST_RETRY_ATTEMPTS; attempt++) {
                ...
                try {
                    ...
                    System.err.println("[Strata] Bot event #" + eventId + " auth retry interrupted.");
                    return "";
                }
            }
            ...
            System.err.println("[Strata] Bot event #" + eventId + " auth request error: " + error.getMessage());
            return "";
        }
    }
    ...
    private static synchronized void cacheAuthToken(String minecraftUuid, String token) {
        ...
        if (cleanToken.equals(StrataState.alertAuthToken)
                && cleanMinecraftUuid.equalsIgnoreCase(StrataState.alertAuthMinecraftUuid)) {
            return;
        }

        StrataState.alertAuthToken = cleanToken;
        StrataState.alertAuthMinecraftUuid = cleanMinecraftUuid;
        StrataConfig.save();
    }

    private static synchronized void clearCachedAuthToken() {
        if (StrataState.alertAuthToken.isEmpty()
                && StrataState.alertAuthMinecraftUuid.isEmpty()) {
            return;
        }

        StrataState.alertAuthToken = "";
        StrataState.alertAuthMinecraftUuid = "";
        StrataConfig.save();
    }
