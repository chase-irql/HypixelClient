package io.github.chaseirql.hypixelclient.client.combat;
...
public final class AutoClicker {
    ...
    public static void tick(Minecraft client) {
        if (!HypixelClientState.autoClickerEnabled
                || !HypixelClient.isPlayerActive(client)
                || client.level == null
                || client.player == null
                || client.gameMode == null
                || !client.options.keyAttack.isDown()
                || (HypixelClientState.autoSwingEnabled && HypixelClientState.lockedTarget != null)
    ...
    public static boolean shouldFireAutoSwing() {
        long now = System.currentTimeMillis();
        float minCps = delayMsToCps(HypixelClientState.swingDelayMax);
        float modeCps = delayMsToCps(HypixelClientState.swingDelayMode);
        float maxCps = delayMsToCps(HypixelClientState.swingDelayMin);
    ...
    public static void fireAttack(Minecraft client) {
        ...
        if (minecraft.hypixelclient$getMissTime() > 0) {
            minecraft.hypixelclient$setMissTime(0);
        }
        minecraft.hypixelclient$invokeStartAttack();
    }
    ...
    public static void pushClickCps() {
        ...
        if (lastAttackMs > 0) {
            ...
            rawCps = HypixelClientState.acCpsMode;
        }
        lastAttackMs = now;
