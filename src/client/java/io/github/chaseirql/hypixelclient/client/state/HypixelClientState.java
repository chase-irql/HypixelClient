package io.github.chaseirql.hypixelclient.client.state;
...
/**
 * In-memory settings and transient runtime state shared by HypixelClient's client features.
 * Persistent fields are loaded and saved by {@code HypixelClientConfig}; session-only identifiers
 * are deliberately reset when the client disconnects or a feature stops.
 */
public final class HypixelClientState {

    /** Empty by default so a fresh install never sends gameplay data to a third party. */
    public static final String DEFAULT_ALERT_SERVER_URL = "";
    ...
    public static boolean chatNameMentionAlertsEnabled = false;
    public static boolean serverShutdownAlertsEnabled = false;
