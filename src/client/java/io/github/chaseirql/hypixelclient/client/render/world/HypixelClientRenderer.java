package io.github.chaseirql.hypixelclient.client.render.world;
...
/** Builds and renders the world-space boxes used by targeting and Hideonleaf features. */
public final class HypixelClientRenderer {

    private static final RenderPipeline BOX_PIPELINE = RenderPipelines.register(
            RenderPipeline.builder(RenderPipelines.LINES_SNIPPET)
                    .withLocation(Identifier.fromNamespaceAndPath(HypixelClient.MOD_ID, "pipeline/enemy_box_through_walls"))
    ...
    public static void render(Camera camera, Matrix4f viewMatrix, float tickDelta) {
        ...
        // ── Regular ESP boxes ────────────────────────────────────────────────
        if (HypixelClientState.enabled && HypixelClientState.hasTarget()) {
            for (Entity entity : client.level.entitiesForRendering()) {
                if (entity == client.player) continue;
                if (!(entity instanceof LivingEntity living) || !living.isAlive()) continue;
                if (!HypixelClientState.matches(living)) continue;
                ...
                boolean isLocked = HypixelClientState.lockedTarget != null &&
                        HypixelClientState.lockedTarget.equals(entity.getUUID());
        ...
        if (HypixelClientState.autoHuntEnabled) {
            for (Entity entity : client.level.entitiesForRendering()) {
                ...
                boolean isBullet  = entity instanceof ShulkerBullet
                        && HypixelClientState.huntTrackedBullet != null
                        && entity.getUUID().equals(HypixelClientState.huntTrackedBullet);

                if (!isShulker && !isBullet) continue;

                // Skip shulkers that ESP is already drawing to avoid double-draw
                if (isShulker && HypixelClientState.enabled
                        && HypixelClientState.matches((LivingEntity) entity)) continue;
                ...
                if (isShulker) {
                    boolean isActive = HypixelClientState.huntLockedShulker != null
                            && HypixelClientState.huntLockedShulker.equals(entity.getUUID());
                    ...
                    boolean isAimLocked = HypixelClientState.huntLockedBullet != null
                            && HypixelClientState.huntLockedBullet.equals(entity.getUUID());
        ...
        if (HypixelClientState.beachballMacroRunning && beachballRenderId != null && !beachballRendered) {
            for (Entity entity : client.level.entitiesForRendering()) {
        ...
        if (HypixelClientState.autoFisherEnabled && AutoFisher.originBlock != null) {
    ...
    private static void drawBoxes(Minecraft client) {
        ...
        if (vertexBuffer == null || vertexBuffer.size() < needed) {
            ...
            vertexBuffer = new MappableRingBuffer(
                    () -> HypixelClient.MOD_ID + " enemy box vbo",
        ...
        try (RenderPass pass = RenderSystem.getDevice()
                ...
                .createRenderPass(
                        () -> HypixelClient.MOD_ID + " enemy box pass",
