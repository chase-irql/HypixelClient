...
public final class AutoFisher {
    ...
    public static void tick(Minecraft client) {
        ...
        switch (state) {
            case IDLE -> {
                ...
                // (see HypixelClient.toggleAutoFisher). This fallback only fires
    ...
    public static void forceStop(Minecraft client, String reason) {
        if (!HypixelClientState.autoFisherEnabled) return;
        ...
        HypixelClientState.autoFisherEnabled = false;
        reset();
        HypixelClientConfig.save();

        if (client != null && client.player != null) {
            client.player.displayClientMessage(Component.literal(
                    "[HypixelClient] Auto Fish stopped: " + reason
            ), false);
        }
        ...
        if (!alertQueued && client != null && client.player != null) {
            client.player.displayClientMessage(Component.literal(
                    "[HypixelClient] Fishing forced-stop alert not queued. Check bot alert settings."
            ), false);
        }
    }
