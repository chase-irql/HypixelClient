...
/** Selects eligible entities and coordinates lock-on, auto-swing, and automated-use behavior. */
public final class LockOnController {
    ...
    public static void clearLock() {
        HypixelClientState.lockedTarget = null;
        HypixelClientAim.reset();
        resetAutoSwingState();
    }

    public static void updateLockOn(Minecraft client) {
        if (client.level == null || client.player == null) return;
        if (!HypixelClientState.enabled || !HypixelClientState.hasTarget()) return;

        if (HypixelClientState.lockedTarget != null) {
            Entity lockedEntity = null;
            for (Entity entity : client.level.entitiesForRendering()) {
                if (entity.getUUID().equals(HypixelClientState.lockedTarget)) {
                    lockedEntity = entity;
                    break;
                }
            }
            if (lockedEntity instanceof LivingEntity living && living.isAlive()
                    && HypixelClientState.matches(living)
        ...
        for (Entity entity : client.level.entitiesForRendering()) {
            ...
            double fovR = HypixelClientState.lockFov;
        ...
        if (!candidates.isEmpty()) {
            ...
            float blend = HypixelClientState.aimPriorityBlend;
        ...
        HypixelClientState.lockedTarget = bestTarget;
    }
    ...
    public static void tickAutoSwing(Minecraft client) {
        ...
        if (isNewTarget && HypixelClientState.randomizeReactionDelay) {
    ...
    public static void tickAutoHuntUse(Minecraft client) {
        ...
        UUID huntTarget = HypixelClientState.autoHuntEnabled ? HypixelClientState.huntLockedBullet : null;
        if (huntTarget == null) {
            resetHuntUseState();
            return;
        }
    ...
    private static UUID getAutoSwingTarget() {
        ...
        if (FishingCombat.isKilling() && HypixelClientState.lockedTarget != null) {
            return HypixelClientState.lockedTarget;
        }
        if (HypixelClientState.autoSwingEnabled && HypixelClientState.lockedTarget != null) {
            return HypixelClientState.lockedTarget;
        }
        return null;
    }
