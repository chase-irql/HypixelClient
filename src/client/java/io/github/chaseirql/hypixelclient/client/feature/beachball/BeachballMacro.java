package io.github.chaseirql.hypixelclient.client.feature.beachball;
...
import io.github.chaseirql.hypixelclient.client.mixin.accessor.MinecraftAccessor;
...
/** State machine for detecting, positioning beneath, and interacting with beachball entities. */
public final class BeachballMacro {
    ...
    public static void stopIfRunning(Minecraft client) {
        if (!HypixelClientState.beachballMacroRunning) return;
    ...
    public static void tick(Minecraft client) {
        ...
        if (!HypixelClient.isPlayerActive(client)) {
            releaseMovement(client);
        ...
        postRoundReadyMs = 0L;
        ((MinecraftAccessor) client).hypixelclient$invokeStartUseItem();
    ...
    public static List<String> getDebugLines(Minecraft client) {
        ...
        lines.add("Beachball running=" + (HypixelClientState.beachballMacroRunning ? "ON" : "OFF")
                + " state=" + status);
    ...
    public static UUID getRenderBallId(Minecraft client) {
        if (!HypixelClientState.beachballMacroRunning) return null;
    ...
    public static void requestStopAfterCurrentRound(Minecraft client, String reason) {
        ...
        if (client != null && client.player != null) {
            client.player.displayClientMessage(
                    Component.literal("[HypixelClient] " + cleanReason + " Finishing the current beachball before stopping."),
                    false
            );
        }
    }
    ...
    public static boolean shouldOverrideMovement() {
        return HypixelClientState.beachballMacroRunning;
    }
    ...
    public static Input getDesiredInput() {
        boolean crouch = HypixelClientState.beachballMacroRunning && !returningToStart && HypixelClientState.beachballCrouchEnabled;
        return new Input(
                desiredForward,
                desiredBackward,
                desiredLeft,
    ...
    private static void stopMacroAfterCurrentRound(Minecraft client, String reason) {
        ...
        if (client != null && client.player != null) {
            client.player.displayClientMessage(
                    Component.literal("[HypixelClient] Beachball macro stopped (" + sanitizeReason(reason, "Current round finished") + ")"),
                    false
            );
        }
    }
    ...
    private static boolean stopMacro(Minecraft client, String reason, boolean sendAlert) {
        ...
        onRunningStateChanged(client, false);
        if (client != null && client.player != null) {
            client.player.displayClientMessage(Component.literal("[HypixelClient] Beachball stopped (" + reason + ")"), false);
        }
        ...
        if (!alertQueued && client != null && client.player != null) {
            client.player.displayClientMessage(
                    Component.literal("[HypixelClient] Forced-stop alert not queued. Check bot alert settings."),
                    false
            );
        }
