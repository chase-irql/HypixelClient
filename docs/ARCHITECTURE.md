...
Strata is a client-only Fabric mod. It has no dedicated server component and does not add blocks,
...
1. `StrataClient` loads configuration and registers key, chat, render, network, and tick callbacks.
...
4. Renderers read current state and draw world boxes or HUD overlays. They do not own gameplay
   decisions.
5. `StrataConfig` persists user choices. Transient locks, counters, and state-machine phases remain
   in memory.
...
`StrataState` is the canonical store for user-configurable values shared across features. Feature
...
`StrataConfig.Data` mirrors only values intended to survive a restart. The runtime fields are
...
to a feature class. Avoid keeping feature state in a Mixin. All injected methods use the `strata$`
prefix to prevent name collisions.

The active Mixin list is in `src/client/resources/strata.client.mixins.json`. A missing or renamed
