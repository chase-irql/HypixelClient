...
HypixelClient stores settings in `config/hypixelclient.json` below the active Minecraft directory. The file is
created when settings are saved. If `config/enemyboxes.json` exists and the HypixelClient file does not,
HypixelClient imports it and writes a new `hypixelclient.json`; the legacy file is left untouched.
...
`alertServerUrl`. HypixelClient derives two endpoints from that base:
...
During authentication, HypixelClient sends the configured service the player's Minecraft name, UUID,
...
The returned bearer token and UUID are cached in `hypixelclient.json`.
...
When the shard tracker is enabled, HypixelClient fetches Bazaar prices from
`https://api.hypixel.net/skyblock/bazaar`. The request is a public GET and does not include an API
key or HypixelClient identifier.

HypixelClient does not include telemetry, advertising, analytics, or an automatic update checker.
